...
You will be provided with a day's worth of raw memory chunks. 
...
async def append_memory(user_id: str, content: str) -> tuple[str, str, str]:
    ...
    file_path = os.path.join(DATA_DIR, user_id, f"memory.{date_str}.md")
    ...
    return memory_id, formatted_block, date_str
...
async def update_memory(user_id: str, memory_id: str, new_content: str) -> bool:
    # We must find which file contains the memory.
    # Since we don't index it in memory, we scan all memory files for the user.
